...
(if (fboundp 'register-definition-prefixes) (register-definition-prefixes "telega-filter" '("all" "any" "contact" "custom" "define-telega-filter" "has-" "ids" "label" "me-is-member" "mention" "name" "not" "opened" "permission" "pin" "restriction" "saved-messages" "telega-" "top" "tracking" "type" "user-status" "verified")))
...
(defvar telega-url-shorten-mode nil "\
Non-nil if Telega-Url-Shorten mode is enabled.
See the `telega-url-shorten-mode' command
for a description of this minor mode.
Setting this variable directly does not take effect;
either customize it (see the info node `Easy Customization')
or call the function `telega-url-shorten-mode'.")

(custom-autoload 'telega-url-shorten-mode "telega-modes" nil)
